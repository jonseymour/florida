...
The `--girls` flag selects only children who are girls. The reported statistics relate to populations of girls, not families. In particular when `--girls` is specified, the 'gg' statistic can be read as the number of girls whose sibling is also a girl.

The `--sample-and-match` flag samples a population of random families and ramdom daughters many times
and when the samples match on the criteria, outputs the family. When the `--daughters-match` option is true (the default), a match occurs if the sampled family has one daughter and the family's daughter is same as the sampled daughter or if family has 2 daughters and the random choice from those 2 daughters is the same as the sampled daughter. If `--daughters-match` is false, then a match occurs if the family of the sampled daughter is the sampled family.
...
`sampleAndMatch` is a filter that can be used to examine the [Mary, Jane, K problem](MARY-JANE-K.md) problem.
...
Experiment with different values for `--probability` and see how the result changes. For example try at least: 0.001, 0.33, 0.5, 1.
...
Try different values of `--probability`. Does gg/c output change? Compare these results with 1B.
...
Try different values of `--probability`. Does gg/c output change? Compare these results with 1B and 2B.
